/**
 * POST /api/tunjangan/transport/hitung
 * Menghitung tunjangan transport untuk seluruh pegawai tetap (PKWTT) pada bulan tertentu.
 * Formula: nominal = base_fare × km_dibulatkan × hari_kerja
 * Aturan:
 *   - Minimal 19 hari kerja untuk mendapat tunjangan
 *   - Jarak 5-25 km (di luar itu tidak dihitung)
 *   - Pembulatan km: ≥0.5 ke atas, <0.5 ke bawah
 *   - Hanya pegawai tetap (jenis_kontrak = 'PKWTT')
 * Request body: { bulan?: number (1-12, default: bulan saat ini), tahun?: number (default: tahun saat ini) }
 * Auth: JWT (required) — hanya user dengan akses `modul_tunjangan_transport`.
